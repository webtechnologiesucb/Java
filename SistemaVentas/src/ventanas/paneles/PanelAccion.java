...
public class PanelAccion extends javax.swing.JPanel {
	...
	private void bntAgregarActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_bntAgregarActionPerformed
		...
		if (pvc.getProducto() != null) {
			...
			/**
			 * verificar corregir cuamdo se borra la primera fila nos genera un dato nulo.
			 */
			...
			if (!mtdc.existe(pvc.getProducto())) {
				...
				// ((CellEditorSpinnerCompra)this.tabla.getCellEditor(this.indexFila ,
				// 1)).getSpinner().requestFocus();

			} else {
				JOptionPane.showInternalMessageDialog(ifr,
						"El Producto: " + pvc.getProducto().getNombre() + " ya ha sido agregado", "Producto Duplicado",
		...
		if (((DetalleCompra) mtdc.getFila(this.indexFila)).getIdProducto().getPrimaryKey() != null) {
			if (this.tabla.getRowCount() > 1) {
				if (this.indexFila < this.tabla.getRowCount() - 1) {
					int opc = JOptionPane.showInternalConfirmDialog(ifr, "¿Realmente desea quitar este producto?",
							"Quitar Producto", JOptionPane.YES_NO_CANCEL_OPTION);
					if (opc == JOptionPane.OK_OPTION) {
						if (((ICompra) ifr).isEsActualizacion()) {
							DetalleCompra dcompra = (DetalleCompra) ((DetalleCompra) mtdc.getFila(this.indexFila))
									.clone();
			...
			if ((this.indexFila == 0 && this.tabla.getRowCount() == 1)
					|| (this.indexFila == this.tabla.getRowCount() - 1 && this.tabla.getRowCount() > 1)) {
				int opc = JOptionPane.showInternalConfirmDialog(ifr, "¿Realmente desea quitar este producto?",
						"Quitar Producto", JOptionPane.YES_NO_CANCEL_OPTION);
				if (opc == JOptionPane.OK_OPTION) {
					// ((DetalleCompra)mtdc.getFila(this.indexFila)).setIdProducto(new Producto());
