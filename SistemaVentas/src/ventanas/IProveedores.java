...
public class IProveedores extends javax.swing.JInternalFrame {
	...
	private void initComponents() {
		...
		pnlEntradas.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Datos del Proveedor",
	...
	private void bntEliminarActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_bntEliminarActionPerformed
		int opcion = JOptionPane.showInternalConfirmDialog(this, "¿Desea Eliminar este proveedor?", "Desea Eliminar",
				JOptionPane.YES_NO_CANCEL_OPTION);
		if (opcion == JOptionPane.OK_OPTION) {
			// mtp.borrarFila(tblProveedores.getSelectedRow());
			int rs = cp.borrarRegistro(cp.getProveedor());
			if (rs != 0) {
				JOptionPane.showMessageDialog(this, "Regisro Eliminado correctamente", "Registro Eliminado",
	...
	private void bntGuardarActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_bntGuardarActionPerformed
		if (esActualizacion) {
			if (!this.verficarCambios()) {
				JOptionPane.showMessageDialog(this, "Debe cambiar por lo menos algun valor", "No hubo cambios",
						JOptionPane.INFORMATION_MESSAGE);
				return;
			}
		}
	...
	private void bntBuscarActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_bntBuscarActionPerformed
		...
		String msj = (tipoSeleccion == -1 ? "Mostrando todos los Proveedores existentes"
				: (tipoSeleccion == 1 ? "Mostrando todo los usuarios activos"
						: "Mostrando todo los usuarios No activos"));
	...
	private void cerrarVentana() {
		int opcio = JOptionPane.showInternalConfirmDialog(this,
				"Perderá todos los datos" + " que no hayan sido guardados\n" + "¿Desea cerrar esta ventana?",
				"¿Desea cerrar la ventana?", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE);
	...
	private boolean guardarActualizar() {
		...
		if (ECampos.esObligatorio(this.pnlEntradas, true)) {
			JOptionPane.showInternalMessageDialog(this, "Los campos en rojo son obligatorios",
					"Llene los campos obligatorios", JOptionPane.ERROR_MESSAGE);
			return false;
		}
		...
		if (opcion == JOptionPane.OK_OPTION) {
			...
			if (seguardo) {
				...
				JOptionPane.showInternalMessageDialog(this, "Error: no se pudo guardar.",
						"Error al guardar el registro", JOptionPane.ERROR_MESSAGE);
			}
			ECampos.esObligatorio(this.pnlEntradas, false);
		}
